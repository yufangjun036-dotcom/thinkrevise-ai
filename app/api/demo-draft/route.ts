...
export async function POST(request: Request) {
  ...
  if (typeof topic !== "string" || !topic.trim() || topic.length > 600 || !Array.isArray(words) || words.length < 1 || words.length > 10 || new Set(words).size !== words.length || words.some((word) => typeof word !== "string" || !/^[a-zA-Z-]{1,30}$/.test(word)) || typeof previousDraft !== "string" || previousDraft.length > 6000) return json({ error: "Please choose the topic and target words again." }, 400);
  ...
  if (!key) return json({ error: "Demo-draft generation requires live AI. Please try again later or write your own draft." }, 503);
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        validationHint = "The previous request failed. Generate a complete draft again.";
        continue;
      }
      ...
      if (!output) {
        ...
        validationHint = "The previous response did not include a draft. Return a complete JSON result.";
        continue;
      }
      try {
      ...
      validationHint = "The previous request was interrupted. Generate a complete draft again.";
  ...
  return json({ error: validationFailure ? "This demo draft did not pass the target-word or content checks. Please generate it again." : "AI could not generate a demo draft just now. Please try again later." }, 502);
  } finally {
    access.release();
  }
}
